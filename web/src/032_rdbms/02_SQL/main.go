...
func main() {
	// db, err = sql.Open("mysql", "root@cloudsql(avid-atlas-186018:us-central1:gosql)/gosql")
	db, err = sql.Open("mysql", "academy:gosql_academy@/gosql")
	check(err)
	defer db.Close()
...
func insert(w http.ResponseWriter, req *http.Request) {
	...
	stmt, err := db.Prepare(`INSERT INTO customer VALUES ("Diego");`)
	check(err)
	defer stmt.Close()
...
func update(w http.ResponseWriter, req *http.Request) {
	stmt, err := db.Prepare(`UPDATE customer SET name="Hector" WHERE name="Diego";`)
	check(err)
	defer stmt.Close()
...
func del(w http.ResponseWriter, req *http.Request) {
	stmt, err := db.Prepare(`DELETE FROM customer WHERE name="Hector";`)
	check(err)
	defer stmt.Close()
